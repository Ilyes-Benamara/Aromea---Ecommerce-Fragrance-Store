...
$f = ['id'=>0,'name'=>'','brand'=>'','collection_id'=>0,'description'=>'','price'=>'','image_url'=>'','video_url'=>'',
      ...
      'occasion'=>'','bottle_size'=>'','stock'=>0,'is_featured'=>0,'classification'=>'',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$name || $price <= 0) {
        ...
        if ($id > 0) {
            $stmt = $conn->prepare("UPDATE fragrances SET name=?,brand=?,collection_id=?,description=?,price=?,image_url=?,video_url=?,concentration=?,gender=?,longevity=?,sillage=?,occasion=?,bottle_size=?,stock=?,is_featured=?,classification=?,season_summer=?,season_spring=?,season_fall=?,season_winter=?,time_day=?,time_night=?,updated_at=NOW() WHERE id=?");
            $stmt->bind_param('ssissssssssssiisiiiiiii',$name,$brand,$cid,$description,$price,$image_url,$video_url,$concentration,$gender,$longevity,$sillage,$occasion,$bottle_size,$stock,$is_featured,$classification,$ss,$sp,$sf,$sw,$td,$tn,$id);
            if ($stmt->execute()) { $success='Fragrance updated.'; $fid=$id; }
            else $error='Update failed: '.$conn->error;
        } else {
            $stmt = $conn->prepare("INSERT INTO fragrances (name,brand,collection_id,description,price,image_url,video_url,concentration,gender,longevity,sillage,occasion,bottle_size,stock,is_featured,classification,season_summer,season_spring,season_fall,season_winter,time_day,time_night) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            $stmt->bind_param('ssissssssssssiiisiiiii',$name,$brand,$cid,$description,$price,$image_url,$video_url,$concentration,$gender,$longevity,$sillage,$occasion,$bottle_size,$stock,$is_featured,$classification,$ss,$sp,$sf,$sw,$td,$tn);
